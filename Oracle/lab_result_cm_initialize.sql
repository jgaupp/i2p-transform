...
/
create or replace procedure PCORNetLabResultCM(patient_num_first int, patient_num_last int) as
begin
...
and (m.patient_num between patient_num_first and patient_num_last)
;

END PCORNetLabResultCM;
/
insert into cdm_status (status, last_update) select 'lab_result_cm_initialize', sysdate from dual
/
select 1 from cdm_status where status = 'lab_result_cm_initialize'
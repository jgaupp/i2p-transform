...
END;
/
create or replace procedure PCORNetEncounter(patient_num_first int, patient_num_last int) as
begin
...
/
insert into cdm_status (status, last_update) select 'encounter_initialize', sysdate from dual
/
select 1 from cdm_status where status = 'encounter_initialize'